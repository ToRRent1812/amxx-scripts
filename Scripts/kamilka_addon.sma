...
{			
	...
	cvar_revival_time = register_cvar("cod_revkit_time", "2") // Jak d³ugo ma wskrzeszaæ osobê.
	cvar_revival_health = register_cvar("cod_revkit_health", "75") // Ile ma daæ graczowi hp po wskrzeszeniu
	cvar_revival_dis = register_cvar("cod_revkit_distance", 	"100.0") // Z jakiego maksymalnej odleg³oœæ mo¿na wskrzeszaæ
	cvar_za_zbeszczeszczenie = register_cvar("cod_revkit_xpdefiling", "1") // Ile expa ma dawaæ za 'zjedzenie' przeciwnika
	cvar_za_wskrzeszenie = register_cvar("cod_revkit_xpraise", "1") // Ile expa ma dawaæ za wskrzeszenie osoby ze swojej dru¿yny
	cvar_za_hpdefiling = register_cvar("cod_revkit_hpdefiling", "5") // Ile hp ma dawaæ za 'zjedzenie' przeciwnika
	cvar_za_ilerazy = register_cvar("cod_revkit_ile", "3") // Ile osób mo¿na wskrzesiæ na rundê
...
public plugin_natives()
	register_native("cod_add_wskrzes", "native_cod_add_wskrzes", 1);

public native_cod_add_wskrzes(id, ile)
{
	g_haskit[id] = ile;
	ile_ma[id] = 0
}
...
public task_revive(taskid) 
{
	...
	if(g_revive_delay[id] < gametime)
	{
		if(findemptyloc(body, 10.0))
		{
			...
			if(ile_ma[id] == get_pcvar_num(cvar_za_ilerazy))
				off(id)
			...
			if(get_user_team(id)!=get_user_team(lucky_bastard))
			{
				...
				if(get_playersnum() > 2)
				{
					...
					cod_show_exp_reward(id, za_zbeszczeszczenie, "Zniszczenie zwlok");
					//COD_MSG_EXP_P;
					//show_dhudmessage(id, "Zniszczenie zwlok^n+%i", za_zbeszczeszczenie);
				}
			}
			else
			{
				...
				if(get_playersnum() > 2)
				{
					...
					cod_show_exp_reward(id, za_wskrzeszenie, "Wskrzeszenie");
					//COD_MSG_EXP_P;
					//show_dhudmessage(id, "Wskrzeszenie^n+%i", za_wskrzeszenie);
				}
