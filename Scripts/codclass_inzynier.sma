...
new const opis_klasy[] = "Niska Grawitacja, Natychmiastowe zabicie z noza, Posiada dzialko";
new bronie_klasy = 1<<CSW_HEGRENADE | 1<<CSW_FIVESEVEN;
new zdrowie_klasy = 15;
new kondycja_klasy = 40;
new inteligencja_klasy = -15;
...
public plugin_init() 
{
	...
	pcvarHealth = register_cvar("cod_turret_hp","250")
}
...
public plugin_precache()
{
	...
	precache_model("models/QTM_CodMod/v_kdwrench.mdl")
...
{
	if(is_user_bot(id) && cod_get_user_class(id) == cod_get_classid("Inzynier"))
		cod_set_user_class(id, random_num(1, cod_get_classes_num()/2), 1);
...
public ResetHUDg(id)
{
	if(cod_get_user_class(id) == cod_get_classid("Inzynier"))
		set_pev(id, pev_gravity, 0.7);
}
...
public TakeDamage(this, idinflictor, idattacker, Float:damage, damagebits)
{
	...
	if(get_user_weapon(idattacker) == CSW_FIVESEVEN && damagebits & DMG_BULLET)
...
public OpcjeDzialka(id)
{
	...
	gMenuDzialko[id] = menu_create("Dzialko Inzyniera", "OpcjeDziala_Handle");
	menu_additem(gMenuDzialko[id],"Stworz Dzialko");
	menu_additem(gMenuDzialko[id],"Zniszcz Dzialko");
		...
		case 0:
		{
			if(ma_klase[id] && ma_dzialko[id])
			{
				...
				else
				{
					ma_dzialko[id] = false;
					COD_MSG_SKILL_D;
					show_hudmessage(id, "Uderzaj kluczem francuskim o dzialko aby je wybudowac");
				}
			}
		}
		case 1:
...
public ham_ItemDeploy_Post(weapon_ent)
{
	...
	{
		entity_set_string(owner, EV_SZ_viewmodel, "models/QTM_CodMod/v_kdwrench.mdl")
		OpcjeDzialka(owner);
	}
	if(weaponid != CSW_KNIFE && ma_klase[owner])
	{
		client_cmd(owner, "menuselect 0"); 
	}
	return HAM_IGNORED;
}
...
public fw_TraceAttack_Building(id, enemy, Float:damage, Float:direction[3], tracehandle, damagetype){
	...
	if(weapon == CSW_KNIFE && ma_klase[enemy] && pev(id,pev_iuser1) == enemy && equal(classname,"sentry_base") && pev(id,pev_iuser2) < 100){
		set_pev(id,pev_iuser2,pev(id,pev_iuser2)+random_num(7, 10) > 100 ? 100 : pev(id,pev_iuser2)+random_num(7, 10));
		set_animation(enemy,6);
...
public sentry_shot3(ent,target)
{
	...
	if(hit == target)
	{
		...
		ExecuteHam(Ham_TakeDamage, target, 0, pev(ent,pev_iuser1), 15.0 , 1);
