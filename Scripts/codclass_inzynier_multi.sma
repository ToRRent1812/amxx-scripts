...
new ilosc_dzialek_gracza[33];
new max_ilosc_dzialek_gracza[33];
...
{
	register_native("cod_get_user_class_cannon", "PobierzIlosc", 1);
	register_native("cod_get_user_class_max_cannon", "PobierzIloscMax", 1);
}	
...
public cod_class_enabled(id)
{
	...
	ilosc_dzialek_gracza[id] = ILOSC_DZIALEK_GRACZA;
	max_ilosc_dzialek_gracza[id] = ILOSC_DZIALEK_GRACZA;
	ma_klase[id] = true;
}
...
public OpcjeDzialka(id)
{
	...
	menu_additem(gMenuDzialko[id],"Zniszcz wszystkie dzialka");
...
public OpcjeDziala_Handle(id,menu,item)
{
	...
	switch(item)
	{
		case 0:
		{
			if(!ilosc_dzialek_gracza[id])
			{
				client_print(id, print_center, "Nie mozesz postawic juz wiecej dzialek!");
				return PLUGIN_CONTINUE;
			}
...
{
	if(ma_klase[id])
		return ilosc_dzialek_gracza[id]
	return 0;
}
...
{
	if(ma_klase[id])
		return max_ilosc_dzialek_gracza[id]
	return 0;
}