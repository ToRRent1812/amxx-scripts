...
new const
	...
	STR_NOTACTIVE[] 	= "Miny laserowe nie sa aktywne.",
	STR_DONTHAVEMINE[]	= "Nie posiadasz min laserowych.",
	...
	STR_PLANTWALL[]		= "Mine laserowa musisz zamontowac na jakims obiekcie!",
	STR_REF[]		= "Refer to a lasermine rule with this server. say 'lasermine'",
	STR_CBT[]		= "Your Team Can't buy and deploying lasermine!",
	STR_DELAY[]		= "Aby polozyc mine laserowa, poczekaj",
	STR_SECONDS[]		= "sekund.",
	STR_STATE[]		= "Liczba min:",
...
new g_dcount[MAX + 1],g_nowtime,g_MaxPL
new bool:g_settinglaser[MAX + 1]
new g_msgDeathMsg,g_msgScoreInfo,g_msgDamage,g_msgStatusText,g_msgMoney;
new Float:plspeed[MAX + 1]
new plsetting[MAX + 1]
new g_havemine[MAX + 1];
new g_deployed[MAX + 1];
...
public plugin_init() {
	...
	cod_register_class("Aga","Posiada 2 miny laserowe, oraz ma 1/6 szans na wyrzucenie wroga w powietrze",10,0,5,10,0,0,5, "Cycki");
	...
	g_LDMG		= register_cvar("amx_ltm_dmg","40"); 		//laser hit dmg
	g_LFMONEY	= register_cvar("amx_ltm_fragmoney","300");
	g_LHEALTH	= register_cvar("amx_ltm_health","200");
	g_LTMAX		= register_cvar("amx_ltm_teammax","24");
	g_LRADIUS	= register_cvar("amx_ltm_radius","200.0");
	g_LRDMG		= register_cvar("amx_ltm_rdmg","20");		//radius damage
	...
	g_LCBRIGHT	= register_cvar("amx_ltm_bright","30");	//laser line brightness.
	g_LCLMODE	= register_cvar("amx_ltm_color","0"); 		//0 is team color,1 is green
	g_LDMGMODE	= register_cvar("amx_ltm_ldmgmode","1"); 	//0 - frame dmg, 1 - once dmg, 2 - 1 second dmg
	g_LDSEC		= register_cvar("amx_ltm_ldmgseconds","50");	//mode 2 only, damage / seconds. default 1 (sec)
...
public cod_class_enabled(id)
{
	...
	{
		acg_drawtext(id, 0.04, 0.69, "Aby umiescic mine laserowa^npodejdz pod twardy obiekt np.sciane^n i wcisnij USE (domyslnie E)", 0, 212, 255, 255, 0.0, 2.5, 4.5, 0, TS_NONE, 0, 1, 11)
		//COD_MSG_SKILL_D;
		//show_hudmessage(id, "Aby umiescic mine laserowa podejdz pod twardy obiekt np.^nsciane i wcisnij USE (domyslnie E)");
	}
	ma_klase[id] = true;
	//newround(id);
}
...
public Damage(id)
{
	...
	else
	{
		...
		if(ma_klase[idattacker] && random_num(1, 6) == 1)
			set_pev(id, pev_velocity, Float:{0.0, 0.0, 240.0});
	}
...
public Spawn( id )
{
	...
	if(!i_Ent)
	{
		client_print(id,print_chat,"[XP Mod] Can't Create Entity");
...
bool:CanCheck(id,mode)
{
	...
	if( mode == 0)
	{
		if(g_havemine[id] <= 0)
		{
			client_print(id, print_center, "%s",STR_DONTHAVEMINE);
			return false;
		}
	}
...
bool:ReturnCheck( id )
{
	...
	if(g_havemine[id] + 1 > 3) return false;
	new tgt,body,Float:vo[3],Float:to[3];
	get_user_aiming(id,tgt,body);
...
set_score(id,target,hitscore,HP){
	...
	new lvlt = cod_get_user_level(target)
	new lvlk = cod_get_user_level(id)
	new bonus;
	if(lvlt > lvlk)
		bonus = lvlt - lvlk
	else bonus = 0	
	cod_set_user_xp(id, get_cvar_num("cod_killxp")+bonus)
	cod_show_exp_reward(id, get_cvar_num("cod_killxp"), "Frag")
...
ShowAmmo(id)
{ 
    if(g_havemine[id] < 0) return;
    
    new ammo[51] 
    formatex(ammo, 50, "%s %i/2",STR_STATE, g_havemine[id])
