...
new const g_szNazwaKlasy[] = "Mirek";
new const g_szNazwaKodowa[] = "Rasti";
new const g_szOpisKlasy[] = "Ma 3 kupy ktore stawiasz na mapie, 1/5 szans na zadanie 5 dmg wiecej";

new const g_iGrawitacja = -5;
new const g_iZdrowie = 30;
new const g_iKondycja = -5;
new const g_iInteligencja = 0;
new const g_iWytrzymalosc = 10;
...
public plugin_init() {
	...
	register_plugin("Mirek", "0.2b", "MarWit (ToRRent edit)")
	...
	register_touch("kupa", "player",  "DotykMiny");
...
{		
	...
	else
	{
		...
		entity_set_string(ent ,EV_SZ_classname, "kupa");
		...
		set_rendering(ent,kRenderFxNone, 0,0,0, kRenderTransTexture,100);
...
public DotykMiny(ent, id)
{
	...
	if (get_user_team(attacker) != get_user_team(id))
	{
		...
		new entlist[33];
		new numfound = find_sphere_class(ent,"player", 50.0 ,entlist, 32);
		...
		{		
			...
			cod_inflict_damage(attacker, pid, 30.0, 1.0, ent, (1<<24));
			...
			//COD_MSG_NEWS_N;
			acg_drawtext(id, 0.04, 0.69, "Wlazles w kupe !^nPatrz pod nogi", 255, 50, 75, 255, 0.0, 3.0, 3.5, 0, TS_NONE, 0, 1, 12)
...
public TakeDamage(this, idinflictor, idattacker, Float:damage, damagebits)
{
	...
	if(ma_klase[idattacker] && random_num(1,5) == 1)
...
{
	new ent	    
	while((ent = fm_find_ent_by_owner(ent, "kupa", id)) != 0)
		remove_entity(ent)
}
...
{
	new entkupa = find_ent_by_class(0, "kupa");
	while(entkupa > 0)
	{
		if(entity_get_edict(entkupa, EV_ENT_owner) == id)
			remove_entity(entkupa);
		entkupa = find_ent_by_class(entkupa, "kupa");
	}
...
public NowaRunda()
{
	new entkupa = find_ent_by_class(-1, "kupa");
	while(entkupa > 0)
	{
		remove_entity(entkupa);
		entkupa = find_ent_by_class(entkupa, "kupa");
...
{ 
    new ammo[51] 
    formatex(ammo, 50, "Liczba Kup: %i/3",ilosc_kup[id])
