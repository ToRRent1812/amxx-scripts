...
new const perk_name[] = "Bazooka";
new const perk_desc[] = "Dostaje Bazooke za P228";
...
public plugin_init() 
{
	...
	register_cvar("cod_law_rockets", "5");
}
...
{
	COD_MSG_SKILL_D;
	show_hudmessage(id, "Dostajesz Bazooke zamiast P228");
	ma_perk[id] = true;
	Odrodzenie(id)
...
public fw_Touch(ent, id)
{
	...
	new Float:g_damage = random_float(45.0,80.0);
	new Float:g_radius = get_cvar_float("cod_law_radius")+cod_get_user_intelligence(id);
...
stock UTIL_Kill(atakujacy, obrywajacy, Float:damage, damagebits)
{
	...
	cod_inflict_damage(atakujacy, obrywajacy, damage, 0.1, atakujacy, damagebits);
}
